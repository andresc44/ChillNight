const accessToken =
    "ya29.a0AcM612wQTUcri1zbZ89chS_j9CP3GuSpU0djuzQsxoTuuffHkpIBfTRj3QCMtuKYS0RMoGBpPmfn8kvDc4Jhkj9njDuiJtkaWeqs40gBxA-FP9uX-pQaC5Qcce8h4GVchVkjnUeXPbtbhI-oBR35uCZI96duwr0K-58aCgYKAW8SARESFQHGX2MiK9vQWsJS01LJKK8GYldsDg0170"; // Your actual access token
...
async function fetchDrinkCount(name) {
    const response = await fetch(
        `https://sheets.googleapis.com/v4/spreadsheets/1kRCJhcXT5vEIGGkxMxfMNhQMRldbA1m4r5IuyxcKNCA/values/'DrinksData'!A1:B6?access_token=${accessToken}`,
    );
...
async function updateDrinkCount(name, change) {
    ...
    const updateResponse = await fetch(
        `https://sheets.googleapis.com/v4/spreadsheets/1kRCJhcXT5vEIGGkxMxfMNhQMRldbA1m4r5IuyxcKNCA/values/'DrinksData'!B${rowIndex}?valueInputOption=USER_ENTERED`,
        {
            method: "PUT",
            headers: {
...
document.getElementById("submitButton").addEventListener("click", async () => {
    ...
    const drinkCount =
        document.getElementById("drinkCountValue").textContent || "0"; // Get the drink count from the display
    ...
    const response = await fetch(
        `https://sheets.googleapis.com/v4/spreadsheets/1kRCJhcXT5vEIGGkxMxfMNhQMRldbA1m4r5IuyxcKNCA/values/'RawData'!A:E:append?valueInputOption=USER_ENTERED`,
        {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
